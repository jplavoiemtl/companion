...
function adapt(s){return s.replace(/Mode::/g,'Mode.').replace(/diagnet::event/g,'event').replace(/const bool /g,'const ').replace(/nullptr/g,'null').replace(/if \(const char\* reason = entryRefusal\(\)\) \{/,'const reason=entryRefusal(); if(reason) {');}
...
test('entry, repeated on, off and repeated off have explicit results',c=>{c.logRetrievalCommand('log mode on');assert.equal(c.mode,2);c.t++;c.logRetrievalCommand('log mode on');assert.equal(c.lastReason,'not_off');assert.equal(c.activityAt,100);c.logRetrievalCommand('log mode off');assert.equal(c.mode,3);assert.ok(c.logRetrievalActive());c.logRetrievalCommand('log mode off');assert.equal(c.reports.at(-1).result,'already_stopping');c.logRetrievalTick();assert.equal(c.mode,0);c.logRetrievalCommand('log mode off');assert.equal(c.reports.at(-1).result,'already_off');});
...
test('STOPPING retains exclusion until writer release and posts abort only once',c=>{c.enter();c.busy=true;c.logRetrievalExit('usb_command');c.logRetrievalTick();c.logRetrievalExit('other');assert.equal(c.aborts,1);assert.equal(c.mode,3);c.enter();assert.equal(c.lastReason,'not_off');c.busy=false;c.logRetrievalTick();assert.equal(c.mode,0);assert.equal(c.lastReason,'usb_command');});
test('USB loss exits immediately without waiting for an active reader',c=>{c.enter();c.busy=true;c.vbusPresent=false;c.logRetrievalTick();assert.equal(c.mode,3);assert.equal(c.aborts,1);assert.equal(c.exitReason,'usb_power_lost');});
test('logger close and failure exit',c=>{c.enter();c.closing=true;c.logRetrievalTick();assert.equal(c.mode,0);assert.equal(c.lastReason,'logger_closing');c.closing=false;c.enter();c.ready=false;c.logRetrievalTick();assert.equal(c.lastReason,'logger_unavailable');});
test('link loss/recovery retains ACTIVE and does not reset activity',c=>{c.enter();c.t=100000;c.wifi=false;c.logRetrievalTick();assert.equal(c.mode,2);assert.equal(c.linkUp,false);c.wifi=true;c.logRetrievalTick();assert.equal(c.linkUp,true);assert.equal(c.activityAt,100);});
test('idle expires at five minutes even with transfer active',c=>{c.enter();c.t=300099;c.logRetrievalTick();assert.equal(c.mode,2);c.busy=true;c.t++;c.logRetrievalTick();assert.equal(c.mode,3);assert.equal(c.exitReason,'idle_timeout');assert.equal(c.aborts,1);});
test('touch resets idle; status, repeated on and progress do not',c=>{c.enter();c.t=200000;c.logRetrievalTouch();assert.equal(c.activityAt,200000);c.t=400000;c.logRetrievalCommand('log mode status');c.logRetrievalCommand('log mode on');assert.equal(c.activityAt,200000);c.t=500000;c.logRetrievalTick();assert.equal(c.mode,0);});
test('shutdown exit is nonwaiting, does not print or poll for OFF',c=>{c.enter();c.busy=true;c.reports=[];c.logRetrievalExit('shutdown');assert.equal(c.mode,3);assert.equal(c.reports.length,0);assert.doesNotMatch(body(source,'void logRetrievalExit('),/while|delay\(|USBSerial|report\(/);});
...
test('stuck release escalates once at threshold while retaining exclusion',c=>{
 c.enter();c.busy=true;c.logRetrievalExit('usb_command');
...
test('late release recovers after escalation and a new mode resets warning',c=>{
 c.enter();c.busy=true;c.logRetrievalExit('usb_command');c.t+=c.RELEASE_WARN_MS;c.logRetrievalTick();
 c.busy=false;c.logRetrievalTick();assert.equal(c.mode,0);assert.equal(c.lastReason,'usb_command');
 c.enter();assert.equal(c.releaseWarned,false);c.busy=true;c.logRetrievalExit('idle_timeout');
...
test('release at warning deadline completes without a false stuck record',c=>{
 c.enter();c.busy=true;c.logRetrievalExit('usb_command');c.t+=c.RELEASE_WARN_MS;c.busy=false;
