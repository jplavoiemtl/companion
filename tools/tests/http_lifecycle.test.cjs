...
.replace(/diagtime::/g,'diagtime.').replace(/diagtransfer::/g,'diagtransfer.').replace(/Phase::/g,'Phase.').replace(/const Shared (\w+) =/g,'const $1 =')
...
test('full listing fits bounded PSRAM allocation and sends nonchunked',()=>{assert(256*224+2048+1<=65536);assert.match(http,/PAGE_CAP = 65536/);assert.match(http,/heap_caps_malloc\(PAGE_CAP,MALLOC_CAP_SPIRAM \| MALLOC_CAP_8BIT\)/);assert.match(http,/httpd_resp_send\(req,page,used\)/);assert.doesNotMatch(http,/httpd_resp_send_chunk|char page\[/);});
...
test('actual formatter handles 256 maximum-width entries and releases pin on overflow',()=>{
 const c={diagtime:{format:(t,out)=>out.value='2099-12-31 23:59:59 +14:00 approx'},diagtransfer:{last:()=>({id:18446744073709551615n,result:'writer_close_pending',bytes:18446744073709551615n,expected:18446744073709551615n,crc:4294967295,writerBytes:18446744073709551615n,writerCrc:4294967295,crcCheck:'unavailable',appends:'resume_failed',pausedAt:18446744073709551615n,readerClosedAt:18446744073709551615n,resumedAt:18446744073709551615n})},PAGE_CAP:65536,t:9999,pins:0,text:'',limit:65536,entries:Array.from({length:256},()=>({file:{number:99999999,size:18446744073709551615n},times:{opened:{},last:{},fragment:true}}))};
 ...
 .replace(/size_t i=/g,'let i=').replace(/char name\[24\];/g,'let name = {};').replace(/sizeof\(name\)/g,'24').replace('char opened[48], last[48];','let opened={},last={};').replace(/sizeof\((opened|last)\)/g,'48');
 vm.runInContext(`function formatPage(resultOnly,used){${b}}`,c);
 const used={value:0};assert.equal(c.formatPage(false,used),true);assert(used.value<=59392);assert(used.value<65536);assert.equal((c.text.match(/archive-99999999.log/g)||[]).length,256);assert.equal(c.pins,0);
