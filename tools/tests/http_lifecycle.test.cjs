...
.replace(/const (?:bool|int|uint64_t|uint32_t) /g,'const ').replace(/SessionIo\* io =/g,'let io =')
.replace(/Phase::/g,'Phase.').replace(/const Shared (\w+) =/g,'const $1 =')
...
function context(){
 ...
 cancel:false,steps:[],calls:[],napHook:null};
...
test('incidental routes do not reserve or read files and every response closes',()=>{const b=body(http,'esp_err_t handle(');assert.match(b,/405 Method Not Allowed/);assert.match(b,/204 No Content/);assert.match(b,/503 Service Unavailable/);assert.match(b,/404 Not Found/);assert.doesNotMatch(b,/reserve|readChunk|opendir|return ESP_OK/);assert.doesNotMatch(http,/diagreader::(?:view|progress|release|reserve)\(/);});
...
test('actual formatter handles 256 maximum-width entries and releases pin on overflow',()=>{
 const c={PAGE_CAP:32768,t:9999,pins:0,text:'',limit:32768,entries:Array.from({length:256},()=>({size:18446744073709551615n}))};
 ...
 c.append=(used,fmt,...args)=>{let i=0;const out=fmt.replace(/%llu|%s|%u/g,()=>{const a=args[i++];return String(a&&typeof a==='object'&&'value'in a?a.value:a);});if(used.value+out.length>=c.limit)return false;used.value+=out.length;c.text+=out;return true;};
 vm.createContext(c);
