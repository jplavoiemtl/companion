...
Stage 2 is built/flashed by JP: boot-84 startup records, health and download checks pass. JP confirms the prior watchdog followed monitor closure. MQTT off/on and functional hotspot recovery plus Latest pass. RSSI validity and alternating-disconnect suppression fixes pass the boot86 hotspot repeat. Live during controlled MQTT outage passes on boot87. Normal Live comparison also passes: outage FPS is 1.36% below normal. JP accepted Stage 2 on September 20. Stage 3 operation-context logging is prepared, awaiting JP build/bench; optional tail deferred. See the [first Stage 2 case](../src/diagnostics/STAGE2.md).
