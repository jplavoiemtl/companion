...
void logRetrievalUiInit() {
  ...
  labelAt(screen,66,width,"Download logs");
  stateLabel = labelAt(screen,96,width,"Starting...");
  labelAt(screen,148,width,"On your iPhone, open Safari:");
  urlLabel = labelAt(screen,177,width,"");
  lv_obj_t* media = labelAt(screen,218,width,"Latest and Live are paused in this mode.");
  lv_obj_set_style_text_font(media,&lv_font_montserrat_16,0);
  lv_obj_t* idle = labelAt(screen,252,width,"Closes after 5 minutes without activity.");
  lv_obj_set_style_text_font(idle,&lv_font_montserrat_16,0);
